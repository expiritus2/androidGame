...
public class Gameplay implements Screen, ContactListener {
    ...
    void checkPlayerBounds() {
        ...
        if (player.getX() - 25 > GameInfo.WIDTH || player.getX() + 125 < 0) {
            if(!player.isDead()){
                playerDied();
            }
        }
    ...
    void playerDied(){
        ...
        if(GameManager.getInstance().lifeScore < 0){
            ...
            GameManager.getInstance().checkForNewHightsores();
...
//        debugRenderer.render(world, box2DCamera.combined);
    ...
    public void beginContact(Contact contact) {
        ...
        if (body1.getUserData() == "Player" && body2.getUserData().equals("Dark Cloud")) {
            if(!player.isDead()){
                playerDied();
            }
        }
